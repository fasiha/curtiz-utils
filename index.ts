 ...
 */
export function generateContextClozed(left: string, cloze: string,
                                      right: string): {left: string; cloze: string; right: string;} {
 ...
 */
export function substringInArray(v: string[], target: string): undefined|{startIdx: number, endIdx: number} {
  ...
  if (match >= 0) {
    ...
    if (startIdx >= 0 && endIdx >= 0) { return {startIdx, endIdx}; }
  }
}
